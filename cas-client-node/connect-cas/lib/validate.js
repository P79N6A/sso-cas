...
function validate(req, res, callback, options) {
  ...
  if (ticket) {
    ...
    validateTicket(req, options, function(err, response, username) {
      /* istanbul ignore if */
      if (err) {
        return callback(function(req, res, next) {
          res.status(500).send({
...
function validateTicket(req, options, callback) {
  ...
  utils.getRequest(casServerValidPath, function(err, response) {
    ...
    var str = response.body.replace(/:?cas:?/g, '');
    var json = {};
    json = xml2json.parser(str);
    //fix-liuran 添加username
    if(json['serviceResponse'] && json['serviceResponse']['authenticationSuccess']){
      var username = json['serviceResponse']['authenticationSuccess']['user'];
      console.log("username:" + username);
      req.session.usercode = username;
    }
